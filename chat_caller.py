...
def initialize_chat(llm_provider):
    ...
    if llm_provider == "azure":
            ...
            if os.getenv("AZURE_OPENAI_CUSTOM_BACKEND") is not None:
                if request.url.path == "/chat/completions":
                    request.url = request.url.copy_with(
                        path=os.getenv("AZURE_OPENAI_CUSTOM_BACKEND")
                    )
        ...
        chat = ChatOpenAI(
            ...
            default_headers=(
                {
                    os.getenv("AZURE_OPENAI_CUSTOM_HEADER"): os.environ.get(
                        "OPENAI_API_KEY"
                    )
                }
        ...
        raise ValueError(
            "LLM-provider not recognized. Check LLM_PROVIDER environment variable."
        )
...
def check_quota_status():
    try:
        daily_calls_sum = get_daily_calls(
            f"{log_path}/call_log_{datetime.now().strftime('%Y-%m-%d')}.log"
        )
...
if os.getenv("VECTOR_STORE") is None or os.getenv("VECTOR_STORE") == "faiss":
    ...
    client = QdrantClient(
        url=os.getenv("VECTOR_STORE_ENDPOINT"),
...
def query_gpt_chat(query: str, history):
    ...
    if current_model == "END":
        return (
            None,
            "I've been dealing with so many requests today that I need to rest a bit. Please come back tomorrow!",
        )
